...
// Assuming usersInBackend has 27 users
const TOTAL_USERS = 27; 

export function getMaxPage(pageSize) {
    // Math.ceil rounds up to ensure even a partial page is counted.
    return Math.ceil(TOTAL_USERS / pageSize);
}

export default function getNextUsers(pageNumber, pageSize) {
    return usersInBackend.slice( // slice method to extract part of the usersInBackend array by
        pageNumber * pageSize, // start index (page number starts from 0)
        (pageNumber + 1) * pageSize // end index
    )
}